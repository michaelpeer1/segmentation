...
% all_ROI_names = {'lHCpost','rHCpost','lHCant','rHCant'};
...
use_nonparametric_stats = 1;        % Use Wilcoxon tests; otherwise, use t-tests
...
use_cross_validation = 0;           % Use cross-validation (across-run correlations); otherwise, average patterns across runs
use_Euclidean_distance = 0;         % Use Euclidean distances to compute neural RDMs; otherwsie, use correlation distances
...
% Matrix for closest object under integration vs. under schematization
md=mat_distances; md(find(eye(num_conditions)))=nan; [Vd,Id]=max(md);   % The closest object to each object under integration
mo=mat_overlay; mo(find(eye(num_conditions)))=nan; [Vo,Io]=max(mo);     % The closest object to each object under schematization
mat_closest_object = nan(num_conditions);
for i=1:num_conditions
    mat_closest_object(i,Id(i)) = 1; mat_closest_object(Id(i),i) = 1;
    mat_closest_object(i,Io(i)) = -1; mat_closest_object(Io(i),i) = -1;
end
...
mat_nondiag_closest_object = mat_closest_object(find(tril(ones(num_conditions),-1)));
...
corr_objview_d2minusd1_average_viewdir = nan(length(subj_data_dirs), num_ROIs);
corr_objview_d2minusd1_closest_object = nan(length(subj_data_dirs), num_ROIs);
...
corr_jrd_average_viewdir = nan(length(subj_data_dirs), num_ROIs);
corr_jrd_closest_object = nan(length(subj_data_dirs), num_ROIs);
...
for subj = 1:length(subj_data_dirs)
    ...
    % Reading the average viewing direction matrix
    mat_diff_average_viewdir = load(fullfile(data_parent_dir, 'mat_diff_average_viewdir.mat'));
    mat_diff_average_viewdir = mat_diff_average_viewdir.mat_diff_average_viewdir;
    mat_nondiag_diff_average_viewdir = mat_diff_average_viewdir(find(tril(ones(num_conditions),-1)));
    ...
    for roi = 1:num_ROIs        % Going over ROIs
        ...
        if ~isempty(curr_ROI)
            ...
            if use_cross_validation == 1
                if use_Euclidean_distance == 0
                    % Object viewing runs - calculating correlation between runs (cross-validation), in each day separately
                    corr_ROI_objview_day1 = fisherz(corr(all_betas_ROI_objview(:,:,1), all_betas_ROI_objview(:,:,2), 'rows', 'complete'));                    % Correlation between day 1 patterns, averaged across runs
                    corr_ROI_objview_day2 = fisherz(corr(all_betas_ROI_objview(:,:,3), all_betas_ROI_objview(:,:,4),'rows', 'complete'));                    % Correlation between day 2 patterns, averaged across runs
                else
                    for c1 = 1:num_conditions
                        for c2 = 1:num_conditions
                            corr_ROI_objview_day1(c1,c2) = sqrt(sum((all_betas_ROI_objview(:,c1,1) - all_betas_ROI_objview(:,c2,2)).^2));                    % Correlation between day 1 patterns, averaged across runs
                            corr_ROI_objview_day2(c1,c2) = sqrt(sum((all_betas_ROI_objview(:,c1,3) - all_betas_ROI_objview(:,c2,4)).^2));                    % Correlation between day 1 patterns, averaged across runs
                        end
                    end
                    % Converting to similarity matrix
                    corr_ROI_objview_day1 = max(corr_ROI_objview_day1(:)) - corr_ROI_objview_day1;
                    corr_ROI_objview_day2 = max(corr_ROI_objview_day2(:)) - corr_ROI_objview_day2;
                end
                % Averaging across the triangles
                corr_ROI_objview_day1 = nansum(cat(3, corr_ROI_objview_day1, corr_ROI_objview_day1'), 3) / 2;
                corr_ROI_objview_day2 = nansum(cat(3, corr_ROI_objview_day2, corr_ROI_objview_day2'), 3) / 2;
            else
                ...
                if use_Euclidean_distance == 0
                    ...
                    for c1 = 1:num_conditions
                        for c2 = 1:num_conditions
                            corr_ROI_objview_day1(c1,c2) = sqrt(sum((all_betas_day1(:,c1) - all_betas_day1(:,c2)).^2));                    % Correlation between day 1 patterns, averaged across runs
                            corr_ROI_objview_day2(c1,c2) = sqrt(sum((all_betas_day2(:,c1) - all_betas_day2(:,c2)).^2));                    % Correlation between day 1 patterns, averaged across runs
                        end
                    end
                end
            end
            
            if use_Euclidean_distance == 1
                % Converting to similarity matrix
                corr_ROI_objview_day1 = max(corr_ROI_objview_day1(:)) - corr_ROI_objview_day1;
                corr_ROI_objview_day2 = max(corr_ROI_objview_day2(:)) - corr_ROI_objview_day2;
            end
            ...
            corr_objview_d2minusd1_average_viewdir(subj, roi) = corr(corr_ROI_objview_d2minusd1_nondiag, mat_nondiag_diff_average_viewdir, 'Type', 'Spearman', 'rows', 'complete');
            corr_objview_d2minusd1_closest_object(subj, roi) = corr(corr_ROI_objview_d2minusd1_nondiag, mat_nondiag_closest_object, 'Type', 'Spearman', 'rows', 'complete');
            ...
            if num_runs_jrd > 0
                ...
                if use_cross_validation == 1
                    % JRD - Calculating correlation between runs (leave one out cross-validation)
                    current_corrs = nan(num_conditions, num_conditions, num_runs_jrd);
                    counter = 1;
                    for r1 = 1:num_runs_jrd
                        %                         % Calculating the average of the remaining runs' betas
                        for r2 = r1+1:num_runs_jrd
                            if use_Euclidean_distance == 0
                                % Going over all possible run combinations
                                current_corrs(:,:,counter) = fisherz(corr(all_betas_ROI_jrd(:,:,r1), all_betas_ROI_jrd(:,:,r2), 'rows', 'complete'));
                            else
                                for c1 = 1:num_conditions
                                    for c2 = 1:num_conditions
                                        current_corrs(c1,c2,counter) = sqrt(sum((all_betas_ROI_jrd(:,c1,r1) - all_betas_ROI_jrd(:,c2,r2)).^2));                    % Correlation between day 1 patterns, averaged across runs
                                    end
                                end
                            end
                            counter = counter + 1;
                    ...
                    corr_ROI_jrd = nanmean(current_corrs, 3);
                    % Averaging across the triangles
                    corr_ROI_jrd = nansum(cat(3, corr_ROI_jrd, corr_ROI_jrd'), 3) / 2;
                else
                    ...
                    if use_Euclidean_distance == 0
                        ...
                        for c1 = 1:num_conditions
                            for c2 = 1:num_conditions
                                corr_ROI_jrd(c1,c2) = sqrt(sum((all_betas_jrd(:,c1) - all_betas_jrd(:,c2)).^2));                    % Correlation between day 1 patterns, averaged across runs
                            end
                        end
                    end
                end
                ...
                if use_Euclidean_distance == 1
                    % Converting to similarity matrix
                    corr_ROI_jrd = max(corr_ROI_jrd(:)) - corr_ROI_jrd;
                end
                ...
                if ~isempty(corr_ROI_jrd)
                    ...
                    corr_jrd_average_viewdir(subj, roi) = corr(corr_ROI_jrd_nondiag, mat_nondiag_diff_average_viewdir, 'Type', 'Spearman', 'rows', 'complete');
                    corr_jrd_closest_object(subj, roi) = corr(corr_ROI_jrd_nondiag, mat_nondiag_closest_object, 'Type', 'Spearman', 'rows', 'complete');
...
if use_nonparametric_stats == 1
    for i=1:num_ROIs
        ...
        p2(i) = signrank(corr_jrd_dist_within_quadrant(:,i),[],'tail','right');
        p3(i) = signrank(corr_jrd_dist_within_segment_adj_quad(:,i),[],'tail','right');
        p4(i) = signrank(corr_jrd_dist_between_segments_adj_quad(:,i),[],'tail','right');
        p5(i) = signrank(corr_jrd_dist_diagonal_quadrants(:,i),[],'tail','right');
        p6(i) = signrank(corr_jrd_quadrants_dist(:,i),[],'tail','right');
        p7(i) = signrank(corr_jrd_estimated_dist(:,i),[],'tail','right');
        p8(i) = signrank(corr_jrd_schema(:,i),[],'tail','right');
        p9(i) = signrank(corr_jrd_schema_btwn(:,i),[],'tail','right');
        p9b(i) = signrank(corr_jrd_schema_y_axis_only(:,i),[],'tail','right');
        p10(i) = signrank(corr_jrd_flipping(:,i),[],'tail','right');
        p11(i) = signrank(corr_jrd_flipping_btwn(:,i),[],'tail','right');
        p12(i) = signrank(corr_jrd_flipping_x_axis_only(:,i),[],'tail','right');
        p13(i) = signrank(corr_jrd_overlay(:,i),[],'tail','right');
        p14(i) = signrank(corr_jrd_overlay_btwn(:,i),[],'tail','right');
        p15(i) = signrank(corr_jrd_overlay_x_axis_only(:,i),[],'tail','right');
        p16(i) = signrank(corr_jrd_segments(:,i),[],'tail','right');
        p17(i) = signrank(corr_jrd_quadrants(:,i),[],'tail','right');
        p18(i) = signrank(corr_jrd_orth_segments(:,i),[],'tail','right');
        p19(i) = signrank(corr_jrd_y_axis(:,i),[],'tail','right');
        p20(i) = signrank(corr_jrd_x_axis(:,i),[],'tail','right');
        p21(i) = signrank(corr_jrd_y_axis_within_segment(:,i),[],'tail','right');
        p22(i) = signrank(corr_jrd_x_axis_within_segment(:,i),[],'tail','right');
        p23(i) = signrank(corr_jrd_y_axis_between_segments(:,i),[],'tail','right');
        p24(i) = signrank(corr_jrd_x_axis_between_segments(:,i),[],'tail','right');
        p25(i) = signrank(corr_jrd_dist_within_segment(:,i),[],'tail','right');
        p26(i) = signrank(corr_jrd_dist_between_segments(:,i),[],'tail','right');
        p27(i) = signrank(corr_jrd_schema_btwn_seg_adj_quad(:,i),[],'tail','right');
        p28(i) = signrank(corr_jrd_flipping_btwn_seg_adj_quad(:,i),[],'tail','right');
        p29(i) = signrank(corr_jrd_overlay_btwn_seg_adj_quad(:,i),[],'tail','right');
    end
else
...
pp_diff_overlay_distance_jrd = [];
for i=1:num_ROIs
    if use_nonparametric_stats == 1
        pp_diff_overlay_distance_jrd(i) = signrank(corr_jrd_distances(:,i),corr_jrd_overlay(:,i));
    else
        [~,pp_diff_overlay_distance_jrd(i)] = ttest(corr_jrd_distances(:,i),corr_jrd_overlay(:,i));
    end
end
...
if use_nonparametric_stats == 1
    for i=1:num_ROIs
        ...
        p2(i) = signrank(corr_objview_d2minusd1_dist_within_quadrant(:,i),[],'tail','right');
        p3(i) = signrank(corr_objview_d2minusd1_dist_within_segment_adj_quad(:,i),[],'tail','right');
        p4(i) = signrank(corr_objview_d2minusd1_dist_between_segments_adj_quad(:,i),[],'tail','right');
        p5(i) = signrank(corr_objview_d2minusd1_dist_diagonal_quadrants(:,i),[],'tail','right');
        p6(i) = signrank(corr_objview_d2minusd1_quadrants_dist(:,i),[],'tail','right');
        p7(i) = signrank(corr_objview_d2minusd1_estimated_dist(:,i),[],'tail','right');
        p8(i) = signrank(corr_objview_d2minusd1_schema(:,i),[],'tail','right');
        p9(i) = signrank(corr_objview_d2minusd1_schema_btwn(:,i),[],'tail','right');
        p9b(i) = signrank(corr_objview_d2minusd1_schema_y_axis_only(:,i),[],'tail','right');
        p10(i) = signrank(corr_objview_d2minusd1_flipping(:,i),[],'tail','right');
        p11(i) = signrank(corr_objview_d2minusd1_flipping_btwn(:,i),[],'tail','right');
        p12(i) = signrank(corr_objview_d2minusd1_flipping_x_axis_only(:,i),[],'tail','right');
        p13(i) = signrank(corr_objview_d2minusd1_overlay(:,i),[],'tail','right');
        p14(i) = signrank(corr_objview_d2minusd1_overlay_btwn(:,i),[],'tail','right');
        p15(i) = signrank(corr_objview_d2minusd1_overlay_x_axis_only(:,i),[],'tail','right');
        p16(i) = signrank(corr_objview_d2minusd1_segments(:,i),[],'tail','right');
        p17(i) = signrank(corr_objview_d2minusd1_quadrants(:,i),[],'tail','right');
        p18(i) = signrank(corr_objview_d2minusd1_orth_segments(:,i),[],'tail','right');
        p19(i) = signrank(corr_objview_d2minusd1_y_axis(:,i),[],'tail','right');
        p20(i) = signrank(corr_objview_d2minusd1_x_axis(:,i),[],'tail','right');
        p21(i) = signrank(corr_objview_d2minusd1_y_axis_within_segment(:,i),[],'tail','right');
        p22(i) = signrank(corr_objview_d2minusd1_x_axis_within_segment(:,i),[],'tail','right');
        p23(i) = signrank(corr_objview_d2minusd1_y_axis_between_segments(:,i),[],'tail','right');
        p24(i) = signrank(corr_objview_d2minusd1_x_axis_between_segments(:,i),[],'tail','right');
        p25(i) = signrank(corr_objview_d2minusd1_dist_within_segment(:,i),[],'tail','right');
        p26(i) = signrank(corr_objview_d2minusd1_dist_between_segments(:,i),[],'tail','right');
        p27(i) = signrank(corr_objview_d2minusd1_schema_btwn_seg_adj_quad(:,i),[],'tail','right');
        p28(i) = signrank(corr_objview_d2minusd1_flipping_btwn_seg_adj_quad(:,i),[],'tail','right');
        p29(i) = signrank(corr_objview_d2minusd1_overlay_btwn_seg_adj_quad(:,i),[],'tail','right');
    end
else
...
pp_diff_overlay_distance_objview = [];
for i=1:num_ROIs
    if use_nonparametric_stats == 1
        pp_diff_overlay_distance_objview(i) = signrank(corr_objview_d2minusd1_distances(:,i),corr_objview_d2minusd1_overlay(:,i));
    else
        [~,pp_diff_overlay_distance_objview(i)] = ttest(corr_objview_d2minusd1_distances(:,i),corr_objview_d2minusd1_overlay(:,i));
    end
end
